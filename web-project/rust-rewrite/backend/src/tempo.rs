...
impl TempoService {
    pub fn new(source_root: PathBuf) -> Result<Self, AppError> {
        let client = reqwest::Client::builder()
            ...
            .build()?;
        let calibration_path = source_root
            .join("tempo-prediction")
            .join("cache")
            .join("calibration_params.json");
        let forecast_cache_path = source_root
            .join("tempo-prediction")
            .join("cache")
            .join("temp_forecast.json");
    ...
    async fn fetch_history_map(&self, season: &str) -> Result<HashMap<String, String>, AppError> {
        ...
        match self.client.get(url).header("Accept", "application/json").send().await {
            ...
            Ok(response) => self.cached_history(season, true).await.ok_or_else(|| {
