...
repls = [
    (
        ...
        'dev_info(&pdev->dev, "AirTies- probe: irq0=%d irq1=%d wol_irq=%d\\n",\n'
        "\t priv->irq0, priv->irq1, priv->wol_irq);\n"
        "\tpriv->base = devm_platform_ioremap_resource(pdev, 0);",
    ),
    (
        ["err = register_netdev(dev);", "ret = register_netdev(dev);"],
        'dev_info(&pdev->dev, "AirTies- hw: version=%d txq=%u rxq=%u tdma=0x%x rdma=0x%x words_per_bd=%u\\n",\n'
    ...
    (
        ...
        'netdev_warn(dev, "AirTies- open: requesting irq0=%d irq1=%d phy_if=%d internal_phy=%d\\n",\n'
    ...
    (
        ...
        "status = bcmgenet_intrl2_1_readl(priv, INTRL2_CPU_STAT) &\n\t\t~bcmgenet_intrl2_1_readl(priv, INTRL2_CPU_MASK_STATUS);\n\n"
        "\tif (status)\n"
        '\t\tnetdev_warn(priv->dev, "AirTies- isr1 pre-clear: irq=%d status=0x%x mask=0x%x\\n",\n'
    ...
    (
        ...
        "status = bcmgenet_intrl2_0_readl(priv, INTRL2_CPU_STAT) &\n\t\t~bcmgenet_intrl2_0_readl(priv, INTRL2_CPU_MASK_STATUS);\n\n"
        "\tif (status)\n"
        '\t\tnetdev_warn(priv->dev, "AirTies- isr0 pre-clear: irq=%d status=0x%x mask=0x%x\\n",\n'
        "\t\t\t    irq, status, bcmgenet_intrl2_0_readl(priv, INTRL2_CPU_MASK_STATUS));",
    ),
    (
        'netif_dbg(priv, tx_err, dev, "bcmgenet_timeout\\n");',
        'netif_dbg(priv, tx_err, dev, "bcmgenet_timeout\\n");\n\n'
        '\tnetdev_warn(dev, "AirTies- timeout q=%u: INTRL2_0 stat=0x%x mask=0x%x INTRL2_1 stat=0x%x mask=0x%x\\n",\n'
        "\t\t    txqueue,\n"
        "\t\t    bcmgenet_intrl2_0_readl(priv, INTRL2_CPU_STAT),\n"
        "\t\t    bcmgenet_intrl2_0_readl(priv, INTRL2_CPU_MASK_STATUS),\n"
        "\t\t    bcmgenet_intrl2_1_readl(priv, INTRL2_CPU_STAT),\n"
        "\t\t    bcmgenet_intrl2_1_readl(priv, INTRL2_CPU_MASK_STATUS));\n"
        '\tnetdev_warn(dev, "AirTies- timeout q=%u: TDMA ctrl=0x%x stat=0x%x RDMA ctrl=0x%x stat=0x%x\\n",\n'
        "\t\t    txqueue,\n"
        ...
        '\tnetdev_warn(dev, "AirTies- timeout q=%u: TDMA ring%u prod=0x%x cons=0x%x read=0x%x write=0x%x\\n",\n'
        "\t\t    txqueue, txqueue,\n"
        ...
        '\tnetdev_warn(dev, "AirTies- timeout q=%u: RDMA ring0 prod=0x%x cons=0x%x read=0x%x write=0x%x\\n",\n'
        "\t\t    txqueue,\n"
